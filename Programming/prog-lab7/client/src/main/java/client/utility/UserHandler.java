...
public class UserHandler {
    ...
    private GroupRaw generateGroupUpdate() throws IncorrectInputInScriptException {
        StudyGroupAsker studyGroupAsker = new StudyGroupAsker(userScanner);
        if (fileMode()) studyGroupAsker.setFileMode();
        String name = studyGroupAsker.askQuestion("Хотите изменить название группы?") ?
                studyGroupAsker.askName() : null;
        Coordinates coordinates = studyGroupAsker.askQuestion("Хотите изменить координаты?") ?
                studyGroupAsker.askCoordinates() : null;
        int studentsCount = studyGroupAsker.askQuestion("Хотите изменить количество студентов?") ?
                studyGroupAsker.askStudentsCount() : -1;
        int expelledStudents = studyGroupAsker.askQuestion("Хотите изменить количество отчисленных студентов?") ?
                studyGroupAsker.askExpelledStudents() : -1;
        int averageMark = studyGroupAsker.askQuestion("Хотите изменить средний балл?") ?
                studyGroupAsker.askAverageMark() : -1;
        FormOfEducation formOfEducation = studyGroupAsker.askQuestion("Хотите изменить форму обучения?") ?
                studyGroupAsker.askFormOfEducation() : null;
        Person person = studyGroupAsker.askQuestion("Хотите изменить админа?") ?
